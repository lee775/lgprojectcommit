...
export async function loadEmployeeTreeTableData(result, nodeBeingExpanded, sortCriteria, input) {
    ...
    let attributesOfresponse = ["contents", "object_name", "object_desc", "fnd0InProcess", "ics_subclass_name", "object_type", "checked_out", "owning_user", "owning_group", "last_mod_date", "release_statuses"];
    ...
    _.forEach(response, function (treeNode) {
        if (treeNode.type == "L2_ClsfyMgmtFolder" || treeNode.type == "Folder") {
            ...
            let temp = vmo;
            vmo = treeView.createViewModelTreeNode(vmo.uid, vmo.type, vmo.props.object_name.dbValue, nodeBeingExpanded.levelNdx +1 , nodeBeingExpanded.levelNdx +2, vmo.typeIconURL);
...
export async function addPartData(ctx, data) {
    ...
    if (parentFolderUIDLength == 0) {
        ...
        for(let i = 0; i < newLoaded1.length; i++){
            if(parentFolderUID == newLoaded1[i].uid)
            beforeDataNum = i
...
export async function getChildrenData(ctx,data) {
    ...
    if (popData.dataProviders.partMgmtTreeDataProvider.selectedObjects.length == 1) {
        ...
        let attributesOfuserid = ["object_string", "object_name", "contents", "object_type", "owning_group", "owning_user", "checked_out", "last_mod_date", "release_status_list", "IMAN_specification", "revision_list"];
        ...
        if (popData.dataProviders.partMgmtTreeDataProvider.selectedObjects[0].levelNdx == 0) {
            ...
            let attributesOfspecManageSrc = ["object_name", "contents", "object_type", "owning_group", "owning_user", "l2_model_name", "l2_division", "l2_model_type", "l2_model_maker", "l2_reference_models", "l2_reference_parts",
                "l2_spec_diameter", "l2_spec_length", "l2_spec_weight", "l2_spec_rpm", "l2_spec_voltage", "l2_spec_material", "IMAN_specification", "revision_list"];
            ...
            let attributesOfspecManageSrc = ["object_name", "contents", "object_type", "item_id", "owning_group", "owning_user", "l2_model_name", "l2_division", "l2_model_type", "l2_model_maker", "l2_reference_models", "l2_reference_parts",
                "l2_spec_diameter", "l2_spec_length", "l2_spec_weight", "l2_spec_rpm", "l2_spec_voltage", "l2_spec_material", "IMAN_specification", "revision_list"];
            await com.getProperties(specManageSrc, attributesOfspecManageSrc);
