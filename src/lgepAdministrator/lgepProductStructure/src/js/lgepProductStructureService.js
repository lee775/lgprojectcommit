...
export async function loadEmployeeTreeTableData(result, nodeBeingExpanded, sortCriteria, input) {
    ...
    if(nodeBeingExpanded.type =="L2_ProdStructureRevision"){
       ...
       await com.getProperties(test.bomline,["bl_child_lines","object_name","bl_item_object_name"]);
        ...
        response = tesBom;
        await com.getProperties(response,["bl_child_lines","object_name","bl_item_object_name","bl_rev_item_revision","bl_rev_item_revision_id","bl_item","bl_rev_current_id_uid","bl_rev_revision_list"]);
    ...
    let attributesOfresponse = ["contents", "object_name", "object_desc", "fnd0InProcess", "ics_subclass_name", "object_type", "checked_out", "owning_user", "owning_group", "last_mod_date", "release_statuses","revision_list","item_id","ps_children"];      
    ...
    _.forEach(response, function (treeNode) {
            ...
            if(temp.type == "L2_ProdStructureRevision"){
                ...
                if (temp.props.ps_children.dbValues.length ==0 ) {
                    ...
                    _.forEach(check, function (t) {
                        if (t.type == "BOMLine" ||t.type == "L2_ProdStructureRevision" ||t.type == "L2_ProdStructure"  || treeNode.type == "Item" ||  treeNode.type == "ItemRevision") {
                            resultType = false;
                        }
                    });
        ...
        let temp = vmo;
        // console.log({vmo});
        vmo = treeView.createViewModelTreeNode(vmo.uid, vmo.type, vmo.props.bl_item_object_name.dbValues, nodeBeingExpanded.levelNdx +1 , nodeBeingExpanded.levelNdx +2, vmo.typeIconURL);
        Object.assign(vmo, temp);
...
export async function addModel(ctx,data){
  ...
  msg.show(0, `${data.addModelTitleText.dbValues[0]} 아이템이 생성되었습니다.`, ["닫기"], [
    function () { }
]);
    } else {
    ...
    msg.show(0, `${data.addModelTitleText.dbValues[0]} Bom 객체가 생성되었습니다.`, ["닫기"], [
        function () { }
    ]);
    }
...
export async function deleteItem(){
    ...
    msg.show(1, "아이템을 삭제하시겠습니까?", ["삭제","취소"],
        async function () {
        ...
        msg.show(1, "Bom을 제거하시겠습니까?", ["삭제","취소"],
        async function () {
