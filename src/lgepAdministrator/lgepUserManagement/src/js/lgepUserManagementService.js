...
export async function getL2UserList(data) {
    ...
    await com.getProperties(l2UserList, ["object_name","l2_is_expert", "l2_answer_count", "l2_point", "l2_knowledge_count", "l2_user_name", "l2_user_id", "l2_selected_knowledge_count", "l2_good_count", "l2_expert_coverages"]);
...
export function userSelectedCheck(value) {
    if (value.userDataProvider.selectedObjects.length <= 0) {
        message.show(1, lgepLocalizationUtils.getLocalizedText("lgepTechnicalMessages", "noneSelUser"), ["Close"], [
            function () { }
        ]);
        return {
...
export async function editUserProps(data) {
    ...
    if (isNaN(data.userPoint.dbValue)) {
        message.show(1, lgepLocalizationUtils.getLocalizedText("lgepUserManagementMessages", "onlyNumber"), ["Close"], [
            function () { }
        ]);
        return {
            isNumber: false
    ...
    message.show(0, lgepLocalizationUtils.getLocalizedText("lgepTechnicalMessages", "editSuccess"), ["close"], [
        function () { }
    ]);
...
const processCaseSensitiveNotEqualsFilter = function (columnFilter, countries) {
    return countries.filter(function (country) {
        for (var i = 0; i < columnFilter.values.length; i++) {
            if (!country.props[columnFilter.columnName].isArray) {
                if (columnFilter.values[i] === '' && (!country.props[columnFilter.columnName].uiValues[0] || country.props[columnFilter.columnName].uiValues[0] === null)) {
                    return false;
