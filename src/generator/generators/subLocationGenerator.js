...
var getLocations = function( moduleJson ) {
    if( moduleJson.states ) {
        return Object.keys( moduleJson.states ).filter( function( stateName ) {
            return moduleJson.states[ stateName ].type === 'location';
        } );
    }
    return [];
};
...
module.exports.execute = function() {
    return generatorUtils.getModule().then( function( targetModuleJson ) {
        ...
        return generatorUtils.loadAllJson( [ 'components/activeworkspace/repo/kit/**/module.json', 'repo/kit/**/module.json', 'src/**/module.json' ], getName ).then( function( allModulesMap ) {
            ...
            return generatorUtils.getUserInput( '-loc', msg, isValidLocation ).then( function( location ) {
                ...
                return generatorUtils.getUserInput( '-n', messages.nameInputMsg, isValidSubLocationName ).then( function( sublocation ) {
                    ...
                    targetModuleJson.states[ sublocation ] = {
                        ...
                        data: {
                            ...
                            nameToken: sublocation,
                            context: {
                                search: {
                                    chartTitle: {
                                        source: '/i18n/' + targetModuleJson.name + 'Messages',
                                        key: sublocation + 'ChartTitle'
                                    }
                                }
                            }
                        },
                        dependencies: [ 'js/aw.native.sublocation.controller' ],
                        params: {
                            filter: null
                        },
                        parent: location,
