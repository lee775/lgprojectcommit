...
module.exports.execute = function() {
    return generatorUtils.getModule().then( function( targetModuleJson ) {
        ...
        return generatorUtils.getUserInput( '-n', messages.nameInputMsg ).then( function( iconName ) {
            iconName = `type${iconName}48`;
            if( !targetModuleJson.aliasRegistry[ iconName ] ) {
                targetModuleJson.aliasRegistry[ iconName ] = [];
            }
            return generatorUtils.getUserInput( '-types', messages.typeInputMsg ).then( function( typeInput ) {
