...
export async function controlPage(data) {
  ...
  if (
    data.dataProviders.SPLMTableContxMenuDataProvider.selectedObjects[0].props.object_string.dbValues[0].includes(recentSearches) ||
    data.dataProviders.SPLMTableContxMenuDataProvider.selectedObjects[0].props.object_string.dbValues[0].includes('최근 검색')
  ) {
...
export async function loadrecentSearch(ctx, data) {
  ...
  for (let folder of folders) {
    if (
      folder.props.owning_user.dbValues[0] === ctx.user.uid &&
      (folder.props.object_string.dbValues[0].includes(recentSearches) || folder.props.object_string.dbValues[0].includes('최근 검색'))
    ) {
      recentFolder = folder;
      break;
    }
