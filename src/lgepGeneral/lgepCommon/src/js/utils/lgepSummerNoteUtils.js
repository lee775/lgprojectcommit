    ...
    for (let i = 0; i < dataset.length; i++) {
      ...
      if (stringTemp.includes('<svg')) {
        ...
        for (let i = 0; i < maxCount; i++) {
          ...
          stringTemp = stringTemp.replace(/<svg (.*?) width=/gi, "<svg viewBox='0 0 " + width + ' ' + height + "' preserveAspectRatio='none' width=");
          stringTemp = stringTemp.replace(/preserveAspectRatio='none' width="[^"]+" height="[^"]+"/gi, 'preserveAspectRatio="none" width="100%" height="100%"');
        }
      }
 ...
 * @returns {String} content HTML Tag(SummerNote Code)
 */
async function readHtmlToSummernoteEdit(itemRev) {
  await com.getProperties(itemRev, ['IMAN_specification']);
  let dataset = [];
  let datasetTemp;
  if (itemRev.props.IMAN_specification.dbValues.length > 0) {
...
export async function summerNoteInitialize(eventData) {
  ...
  if (obj.props.l2_content.dbValues[0] != '' && obj.props.l2_content.dbValues[0] != null) {
    ...
    $('#lgepSummerNote').summernote('reset');
    $('#lgepSummerNote').summernote('code', content + '<br>');
  }
}
...
async function uploadFileToDataset(file, dataset) {
  if (!file) {
    return undefined;
  } else if (Array.isArray(file) && !dataset) {
    let datasetArr = [];
    for (let i of file) {
    ...
    for (let i of file) {
      let result;
      if (dataset) {
        let ticket = await getDatasetWriteTickets(dataset);
        result = {};
        result.modelObject = dataset;
        result.ticket = ticket.commitInfo[0].datasetFileTicketInfos[0].ticket;
      } else {
        result = await _createDatasets(file);
      }
      _uploadFile(result, i);
      datasetArr.push(await _commitDatasetFiles(result.modelObject, i, result.ticket));
      return datasetArr;
    ...
    let returnDataset = await _commitDatasetFiles(result.modelObject, file, result.ticket);
...
function _commitDatasetFiles(targetDataset, file, ticket) {
  var deferred = AwPromiseService.instance.defer();
  let selectedtype = [];
  let type = file.ext ? file.ext : file.name.split('.')[file.name.split('.').length - 1];
  if (type != null && type !== '') {
    type = type.toLowerCase();
    if (type === 'bmp') {
      selectedtype = ['Bitmap', 'Bitmap', 'Plain', 'Image'];
...
export async function stringToDataset(name, content, targetDataset, includeExtension) {
  let files = [];
  files.push(
    new File([content], targetDataset ? targetDataset.props.ref_list.uiValues[0] : name + includeExtension ? '.txt' : '', {
      type: 'text',
    }),
  );
  let dataset = await uploadFileToDataset(files, targetDataset);
  return dataset[0];
}

export function getDatasetWriteTickets(dataset) {
  let inputParam = {
    inputs: [
      {
        createNewVersion: true,
        ...
        datasetFileInfos: [
          {
            allowReplace: true,
            clientId: '',
            fileName: dataset.props.object_name.dbValues[0] + '.txt',
            isText: true,
            namedReferencedName: 'Text',
          },
        ],
      },
    ],
