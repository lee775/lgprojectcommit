...
var buildHeaderProperties = function (headerProperties, object, viewerHeaderProps, commands, addCommand) {
  for (var idx in headerProperties.dbValue) {
    ...
    if (idx === '0' && addCommand) {
      eachHeaderProp.commands = [commands.dbValue[2], commands.dbValue[3], commands.dbValue[4], commands.dbValue[5]];
    }
    ...
    .then((success) => {
      if (success) {
        ...
        if (inData && inData.viewerData && !inData.viewerData.headerProperties) {
          ...
          if (inData.viewerData.views && inData.viewerData.views.length > 0 && inData.viewerData.views[0].file.objectID !== '') {
          ...
          if (datasetVM !== null && fileVM !== null) {
            ...
            if (inData.headerProperties1) {
              buildHeaderProperties(inData.headerProperties1, datasetVM, viewerHeaderProperties, inData.commands, true);
            }
            if (inData.headerProperties2) {
              buildHeaderProperties(inData.headerProperties2, fileVM, viewerHeaderProperties, inData.commands, false);
            }
...
export let refetchViewer = function (declViewModel) {
  ...
  return soaService.post('Internal-AWS2-2017-06-DataManagement', 'getViewerData', getViewerDataInput).then(function (response) {
    if (response.output) {
...
function _loadCustomViewer(viewerName, DatasetType, inData, resolve) {
  soaService
    .post(
      ...
      'loadObjects',
      { uids: [appCtx.ctx.selected.uid] },
      {
        types: [
          {
            name: DatasetType,
            properties: [{ name: 'ref_list' }],
          },
        ],
        useRefCount: false,
      },
