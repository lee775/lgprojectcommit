...
import { navigationFmea, pageUnMount, getProductNameByGroup } from 'js/utils/fmeaCommonUtils';
...
function deleteObj(value) {
  message.show(
    1,
    lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'folderDelete'),
    [
...
function checklistOpen(data) {
  ...
  if (value.length > 1 || value == undefined) {
    message.show(1, '한 개만 선택해주세요.', [lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'close')], [function () {}]);
...
const _allSearch = async (entryValue) => {
  const queryResults = await query.executeSavedQuery(prop.QUERY_DFMEA_MASTER, prop.QUERY_DFMEA_MASTER_PRODUCT, entryValue);
  return queryResults;
};
...
export async function editStartOREnd() {
  ...
  if (sequenceData.summerNoteWidthMax.dbValue == '' || sequenceData.summerNoteWidthMax.dbValues[0] == false) {
    ...
    message.show(
      0,
      lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'noSummerNoteWidthMax'),
      [lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'close')],
      [function () {}],
    );
    return;
...
export async function summerNoteImageWidthMax(data) {
  ...
  if (temp.includes('<svg')) {
    ...
    temp = temp.replace(/<svg (.*?) width=/gi, "<svg viewBox='0 0 " + width + ' ' + height + "' preserveAspectRatio='none' width=");
...
export async function checkListCopyDelete(data) {
  ...
  if (data.dataProviders.checkListCopyData.selectedObjects.length < 1) {
    message.show(
      1,
      lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'noSelectCheckList'),
      [lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'cancle')],
      [function () {}],
    );
    return;
  }
  ...
  message.show(
    1,
    selectedTableData.length + lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'itemDeleteMessage'),
    [
      lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'cancle'),
      lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'confirm'),
    ],
    [
      function () {},
      async function () {
...
export async function checkListItemCopy(data) {
  ...
  for (let i = 0; i < selectedCheckList.length; i++) {
    if (selectedCheckList[i].type == 'L2_DFMEA') {
      message.show(1, '아이템 리비전만 선택해주세요.', [lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'close')], [function () {}]);
      return {
        noSelectItem: false,
      };
    }
...
export async function designcheckListTableLoad(value) {
  ...
  await com.getProperties(getCheck, [
    'l2_main_category',
    'l2_middle_category',
    'l2_subcategory',
    'l2_check_content',
...
export function editSequenceCheck() {
  ...
  if (selectItem == undefined) {
    message.show(
      1,
      lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'onlyProcedure'),
      [lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'close')],
      [function () {}],
    );
    return {
...
export async function searchData(value) {
  ...
  if (value != undefined && value != null && value != '') {
    let result = [];
    let searchData = await query.executeSavedQuery(
      lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'General'),
      [lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'generalName')],
      ['*' + value + '*'],
    );
...
export async function createObject(data) {
  ...
  message.show(
    0,
    lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'folderCreationComplete'),
    [lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'close')],
    [function () {}],
  );
  treeReload();
...
export async function selDataSetItem(data) {
  if (!data || !data.type.includes('Folder')) {
    message.show(
      1,
      lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'sequenceAddFolder'),
      [lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'close')],
      [function () {}],
    );
    return {
...
export async function selectCheckItem(data) {
  if (data == undefined || data.type != 'L2_DgnPageRevision') {
    message.show(
      1,
      lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'itemCheck'),
      [lgepLocalizationUtils.getLocalizedText('lgepDesignSequenceMessages', 'close')],
      [function () {}],
    );
    return {
