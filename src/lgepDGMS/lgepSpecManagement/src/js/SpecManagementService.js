...
async function loadEmployeeTreeTableData(result, nodeBeingExpanded, data, ctx) {
    ...
    try {
        let getPropertiesParam = {
            objects: [treeObj],
            attributes: ["object_string", "contents", "object_desc", "fnd0InProcess", "ics_subclass_name", "object_type", "checked_out", "owning_user", "owning_group", "last_mod_date", "release_statuses"]
        };
    ...
    _.forEach(response, function (treeNode) {
        ...
        let temp = vmo;
        vmo = treeView.createViewModelTreeNode(vmo.uid, vmo.type, vmo.props.object_string.dbValue, nodeBeingExpanded.levelNdx + 1, nodeBeingExpanded.levelNdx + 2, vmo.typeIconURL);
...
async function getChildrenData(data, ctx, eventTable) {
    ...
    if (eventTable.selectedObjects.length != 0) {
        ...
        try {
            let getResult = {
                objects: [userid],
                attributes: ["object_string", "object_name", "contents", "object_type", "owning_group", "owning_user", "checked_out", "last_mod_date", "release_status_list", "IMAN_specification"]
        ...
        if (eventTable.selectedObjects[0].childNdx == "2") {
            ...
            try {
                let getResult = {
                    objects: specUid,
                    attributes: ["object_name", "contents", "object_type", "owning_group", "owning_user", "l2_model_name", "l2_division", "l2_model_type", "l2_model_maker", "l2_reference_models", "l2_reference_parts",
                        "l2_spec_diameter", "l2_spec_length", "l2_spec_weight", "l2_spec_rpm", "l2_spec_voltage", "l2_spec_material", "IMAN_specification"]
            ...
            try {
                let getResult = {
                    ...
                    attributes: ["object_string", "object_name", "contents", "object_type", "owning_group", "owning_user", "checked_out", "last_mod_date", "release_status_list", "revision_list", "item_revision", "revision_number", "IMAN_specification"]
            ...
            try {
                let getResult = {
                    objects: getInfoHomeChildren,
                    attributes: ["object_string", "object_name", "revision_list", "contents", "object_type", "owning_group", "owning_user", "checked_out", "last_mod_date", "release_status_list", "revision_list", "item_revision", "revision_number", "IMAN_specification"]
...
async function createSpecData(data) {
    ...
    if (specName == null || specName == undefined || specName == "") {
        ...
        try {
            ...
            let getResult = {
                objects: [objModel],
                attributes: ["object_string", "contents", "object_name", "l2_model_name", "l2_division", "l2_model_type", "l2_model_maker",
                    "l2_reference_models", "l2_reference_parts", "l2_spec_diameter", "l2_spec_length", "l2_spec_weight", "l2_spec_rpm",
                    "l2_spec_voltage", "l2_spec_material"]
        ...
        msg.show(0, `${specName} 아이템이 생성되었습니다.`, ["닫기"], [
            function () { }
        ]);
    }
...
async function specDeleteObject(data, dataProviders) {
    ...
    if (popData.dataProviders.specManagementChildrenDataProvider.selectedObjects.length == 1) {
        msg.show(1, `${popData.dataProviders.specManagementChildrenDataProvider.selectedObjects[0].props.object_string.dbValue} 항목을 삭제하시겠습니까?`, ["삭제", "취소"], [
            async function () {
                try {
                    ...
                    msg.show(0, `${modelChildOjt.props.object_string.dbValue} 항목이 삭제되었습니다.`, ["닫기"], [
                        function () { }
                    ]);
                } catch (err) {
                    //console.log(err);
        ...
        msg.show(1, `${popData.dataProviders.specManagementChildrenDataProvider.selectedObjects.length}개의 항목을 삭제하시겠습니까?`, ["삭제", "취소"], [
            async function () {
                let deleteObjects = [];
                let deleteItemsID = [];
                let deleteItems = [];
                ...
                try {
                    ...
                    if (deleteItemsResult.length == 1) {
                        msg.show(0, `${deleteItemsResult.props.object_string.dbValues[0]} 항목이 삭제되었습니다.`, ["닫기"], [
                            function () { }
                        ]);
                    } else if (deleteItemsResult.length > 1) {
                        msg.show(0, `${deleteItemsResult.length}개의 항목이 삭제되었습니다.`, ["닫기"], [
                            function () { }
                        ]);
                    }
                } catch (err) {
    ...
    if (tableUID != null && treeUIDValue != null) {
        ...
        if (tableUID != null || tableUID != undefined) {
            if (urlParam.split("?").length > 1) {
                ...
                } else if (data.dataProviders.specManagementChildrenDataProvider.selectedObjects.length > 1 || data.dataProviders.specManagementChildrenDataProvider.selectedObjects.length == 0) {
...
async function addFolderInTree(data) {
    ...
    if (popAddData.dataProviders.specManagementDataProvider.selectedObjects.length != 0) {
        ...
        msg.show(0, "'" + folderName + "'" + createFolderNoty, [close], [
            function () { }
        ]);
    } else {
...
async function deleteFolderInTree(data) {
    ...
    const close = locale.getLocalizedText("lgepSpecManagementMessages", "close");
    msg.show(1, deleteFolderNoty, [deleteBtn, close], [
        async function () {
            ...
            let resultObj = [];
            if (popDeleteData.dataProviders.specManagementDataProvider.selectedObjects[0].props.contents.dbValues != null || popDeleteData.dataProviders.specManagementDataProvider.selectedObjects[0].props.contents.dbValues != undefined || popDeleteData.dataProviders.specManagementDataProvider.selectedObjects[0].props.contents.dbValues != "") {
            ...
            msg.show(0, "'" + deleteFolderName + "'" + deleteCompleteNoty, [close], [
                function () { }
            ]);
        },
        function () { }
