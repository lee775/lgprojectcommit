...
export async function showQuestionProp(data, ctx) {
    ...
    if (typeof (history.pushState) != "undefined") {
        history.pushState(null, null, '#/lgepNoticeBoardHelpDesk?question=' + selected.uid);
    }
...
export async function loadAnswerList(data, ctx) {
    ...
    if (selected != undefined) {
        ...
        for (let item of itemList[0].props.L2_AnswerRelation.uiValues) {
            let searchingQuestion = await query.executeSavedQuery("PostSearch", ["L2_object_type", "L2_item_id"], ["L2_AnswerHelpRevision", item]);
            if(searchingQuestion){
            ...
            if (selectedQ.props.owning_user.uiValue == ctx.user.props.object_string.dbValue) {
                ...
                buttonArray.push({
                    ...
                    onClick: async function ($noty) {
                        ...
                        writerDate.uiValue = "";
                        point.uiValue = "";
                        history.pushState(null, null, '#/lgepNoticeBoardHelpDesk');
                        eventBus.publish("qaAnswerList.listUpdated");
...
app.factory('lgepNoticeBoardHelpDeskService', () => exports);