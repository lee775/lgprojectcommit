...
$(document).on("click", "#pageSearchId", function () {
    id = "pageSearchId";
...
$(document).on("click", "#pageSearchingId", function () {
    id = "pageSearchingId";
...
async function awtreeSearching(data, ctx) {
    ...
    for (let i = 0; i < treePageNameArr.length; i++) {
        ...
        let state = false;
        if (treePageNameArr[i].replace(/(\s*)/g, "").toLowerCase().includes(searchValue)) {
            count++;
            state = true;
        }
        if (!state) {
            if (treePageContentStringArr[i].replace(/(\s*)/g, "").toLowerCase().includes(searchValue)) {
                count++;
            }
        }
    }
...
async function createDgnBook(data, file) {
    ...
    if (itemName != "") {
        loding.openWindow();
        let value = vms.getViewModelUsingElement(document.getElementById("designStdData"));
        if (!value.dataProviders.designStandardTreeTableData.selectedObjects[0] || !value.dataProviders.designStandardTreeTableData.selectedObjects[0].type.includes("Folder")) {
            value = defaultFolder;
        } else {
            value = value.dataProviders.designStandardTreeTableData.selectedObjects[0];
        }
        ...
        try {
            ...
            message.show(2, lgepLocalizationUtils.getLocalizedText("L2_DesignStandardMessages", "manualCreateFail") + "\n " + err.message, [lgepLocalizationUtils.getLocalizedText("L2_DesignStandardMessages", "close")], []);
        }
        return {
            state: 1
        };
    } else {
        message.show(0, lgepLocalizationUtils.getLocalizedText("L2_DesignStandardMessages", "noProperty"), [lgepLocalizationUtils.getLocalizedText("L2_DesignStandardMessages", "close")], []);
        return {
            state: 0
        };
    }
...
async function bomlineTreeSet2(forwardCheck) {
    ...
    const L2DesignStandardData = vms.getViewModelUsingElement(document.getElementById("L2DesignStandardData"));
    if (!selectedValue) {
        if (!standardData.dataProviders.designStandardTreeTableData.selectedObjects[0] || standardData.dataProviders.designStandardTreeTableData.selectedObjects[0].type == "Folder") {
            L2DesignStandardData.selectedBook = undefined;
            return;
        }
    }
...
async function loadEmployeeTreeTableData(result, nodeBeingExpanded, sortCriteria, input, ctx) {
    ...
    await com.getProperties(response, ["object_string", "contents", "l2_reference_book", "object_desc", "fnd0InProcess", "ics_subclass_name", "object_type", "checked_out", "owning_user", "owning_group", "last_mod_date", "release_statuses", "object_name", "revision_list"]);
