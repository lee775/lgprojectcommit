...
let nowFrequencyPage = 0;
let showFrequencyPage = 3;
let maxFrequencyPage = 0;
let AllFrequencyPage = [];
let FrequencyListArray = [];

eventBus.publish('start.Dashboard');
    ...
    types: [
      {
        ...
        properties: [{ name: 'object_name' }, { name: 'creation_date' }, { name: 'items_tag' }, { name: 'l2_views' }, { name: 'owning_group' }],
      },
    ],
  };
...
export async function pagingAfterQnaAction(data, ctx) {
  ...
  if (
    num ===
    data.dataProviders.qnaChipDataProvider.viewModelCollection.loadedVMObjects[
      data.dataProviders.qnaChipDataProvider.viewModelCollection.loadedVMObjects.length - 1
    ].labelDisplayName
  ) {
    data.dataProviders.qnaChipDataProvider.viewModelCollection.loadedVMObjects.splice(0);
    for (let pageList of AllQnaPage) {
...
export async function setFrequencyPageNumber(data, ctx) {
  let searchInput = {
    attributesToInflate: ['object_name', 'object_desc'],
    maxToLoad: -1,
    maxToReturn: -1,
  ...
  FrequencyListArray = listArray.searchResults;
  FrequencyListArray.sort((a, b) => new Number(b.props.l2_views.dbValues[0]) - new Number(a.props.l2_views.dbValues[0]));

  let frequencyListTop10 = FrequencyListArray.slice(0, 10);
  // frequencyListTop10.splice(3);

  let num = Math.ceil(frequencyListTop10.length / showFrequencyPage);
  maxFrequencyPage = num > 15 ? 10 : 5;
  let pageFrequencyResponse = [];
  for (let i = 1; i <= num; i++) {
    pageFrequencyResponse.push({
      chipType: 'SELECTION',
      labelDisplayName: String(i),
    });
  }
  ...
  AllFrequencyPage = [];
  for (let i = 0; i < pageFrequencyResponse.length; i += maxFrequencyPage) AllFrequencyPage.push(pageFrequencyResponse.slice(i, i + maxFrequencyPage));

  pageFrequencyResponse = AllFrequencyPage[0];
  let frequencyListArrayPage = frequencyListTop10.slice(0, showFrequencyPage);
  data.dataProviders.frequentlyList.viewModelCollection.setViewModelObjects(frequencyListArrayPage);
  eventBus.publish('paging.FrequencylistLoaded');

  if (pageFrequencyResponse === undefined || pageFrequencyResponse.length <= 1) {
    data.beforeFrequency.uiValue = '';
    data.firstPageFrequency.uiValue = '';
    data.afterFrequency.uiValue = '';
    data.lastPageFrequency.uiValue = '';
  } else {
    data.afterFrequency.uiValue = '>';
    data.lastPageFrequency.uiValue = '≫';
  }
...
export async function clickedFrequencyPageAction(data, ctx, chip) {
  if (chip.labelDisplayName != '1') {
    data.beforeFrequency.uiValue = '<';
    data.firstPageFrequency.uiValue = '≪';
  } else {
  ...
  if (chip.labelDisplayName != AllFrequencyPage[AllFrequencyPage.length - 1][AllFrequencyPage[AllFrequencyPage.length - 1].length - 1].labelDisplayName) {
    data.afterFrequency.uiValue = '>';
    data.lastPageFrequency.uiValue = '≫';
  } else {
    data.afterFrequency.uiValue = '';
  ...
  nowFrequencyPage = Number(chip.labelDisplayName) * showFrequencyPage - showFrequencyPage;
  ...
  let pageNum = new Number(data.dataProviders.frequencyChipDataProvider.selectedObjects[0].labelDisplayName);
  let FrequencyListArr = [];
  if (pageNum == 1) {
    FrequencyListArray.splice(3);
    data.dataProviders.frequentlyList.viewModelCollection.setViewModelObjects(FrequencyListArray);
  } else if (pageNum > 1) {
    FrequencyListArr = FrequencyListArray.splice((pageNum - 1) * 3);
    FrequencyListArr.splice(3);
    data.dataProviders.frequentlyList.viewModelCollection.setViewModelObjects(FrequencyListArr);
  }

  for (let chips of data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects) {
    chips.selected = false;
    if (chips.labelDisplayName === chip.labelDisplayName) {
      chips.selected = true;
    }
...
export async function pagingBeforeFrequencyAction(data, ctx) {
  let num;
  let idx = -2;
  for (let chip of data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects) {
    idx += 1;
    if (chip.selected) {
      num = Number(chip.labelDisplayName) - 1;
      chip.selected = false;
  ...
  if (num < data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects[0].labelDisplayName) {
    data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects.splice(0);
    for (let pageList of AllFrequencyPage) {
      if (num === Number(pageList[maxFrequencyPage - 1].labelDisplayName)) {
        for (let page of pageList) {
          data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects.push(page);
        }
        break;
      }
    }
    data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects[maxFrequencyPage - 1].selected = true;
  } else {
    data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects[idx].selected = true;
  }

  nowFrequencyPage = num * showFrequencyPage - showFrequencyPage;
  let searchInput = {
    attributesToInflate: ['object_name', 'object_desc'],
    maxToLoad: -1,
    maxToReturn: -1,
  ...
  FrequencyListArray = listArray.searchResults;
  FrequencyListArray.sort((a, b) => new Number(b.props.l2_views.dbValues[0]) - new Number(a.props.l2_views.dbValues[0]));
  let freqListArr = FrequencyListArray.splice(nowFrequencyPage);
  freqListArr.splice(3);
  data.dataProviders.frequentlyList.viewModelCollection.setViewModelObjects(freqListArr);

  eventBus.publish('paging.FrequencylistLoaded');
}

export async function pagingAfterFrequencyAction(data, ctx) {
  let num;
  let idx = 0;
  for (let chip of data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects) {
    idx += 1;
  ...
  if (Number(num) + 1 === Number(AllFrequencyPage[AllFrequencyPage.length - 1][AllFrequencyPage[AllFrequencyPage.length - 1].length - 1].labelDisplayName)) {
    data.afterFrequency.uiValue = '';
    data.lastPageFrequency.uiValue = '';
  }
  data.beforeFrequency.uiValue = '<';
  ...
  if (
    num ===
    data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects[
      data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects.length - 1
    ].labelDisplayName
  ) {
    data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects.splice(0);
    for (let pageList of AllFrequencyPage) {
    ...
    data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects[0].selected = true;
  } else {
    data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects[idx].selected = true;
  }

  nowFrequencyPage = num * showFrequencyPage;
  let searchInput = {
    attributesToInflate: ['object_name', 'object_desc'],
    maxToLoad: -1,
    maxToReturn: -1,
...
export async function firstPageFrequencyAction(data, ctx) {
  for (let chip of data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects) {
    if (chip.selected) {
      chip.selected = false;
      break;
  ...
  for (let page of AllFrequencyPage[0]) {
    data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects.push(page);
  }
  data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects[0].selected = true;
...
export async function lastPageFrequencyAction(data, ctx) {
  for (let chip of data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects) {
    if (chip.selected) {
      chip.selected = false;
      break;
  ...
  nowFrequencyPage =
    (Number(AllFrequencyPage[AllFrequencyPage.length - 1][AllFrequencyPage[AllFrequencyPage.length - 1].length - 1].labelDisplayName) - 1) * showFrequencyPage;
  let searchInput = {
    attributesToInflate: ['object_name', 'object_desc'],
    maxToLoad: -1,
    maxToReturn: -1,
  ...
  let maxPageNum;
  let frequencyListTop10 = FrequencyListArray.slice(0, 10);
  if (frequencyListTop10.length % 3 == 0) {
    maxPageNum = Math.floor(frequencyListTop10.length / 3);
  } else if (frequencyListTop10.length % 3 != 0) {
    maxPageNum = Math.floor(frequencyListTop10.length / 3) + 1;
  }

  let frequencyListArr = frequencyListTop10.splice(frequencyListTop10.length - (frequencyListTop10.length - (maxPageNum - 1) * 3));
  data.dataProviders.frequentlyList.viewModelCollection.setViewModelObjects(frequencyListArr);

  data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects.splice(0);
  for (let page of AllFrequencyPage[AllFrequencyPage.length - 1]) {
    data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects.push(page);
  }
  data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects[
    data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects.length - 1
...
export async function pagingAfterAllAction(data, ctx) {
  ...
  if (
    num ===
    data.dataProviders.searchChipDataProvider.viewModelCollection.loadedVMObjects[
      data.dataProviders.searchChipDataProvider.viewModelCollection.loadedVMObjects.length - 1
    ].labelDisplayName
  ) {
    data.dataProviders.searchChipDataProvider.viewModelCollection.loadedVMObjects.splice(0);
    for (let pageList of AllPage) {
...
export async function pagingAfterTechDocAction(data, ctx) {
  ...
  if (
    num ===
    data.dataProviders.techDocChipDataProvider.viewModelCollection.loadedVMObjects[
      data.dataProviders.techDocChipDataProvider.viewModelCollection.loadedVMObjects.length - 1
    ].labelDisplayName
  ) {
    data.dataProviders.techDocChipDataProvider.viewModelCollection.loadedVMObjects.splice(0);
    for (let pageList of AllTechDocPage) {
...
export async function lastPageTechDocAction(data, ctx) {
  ...
  nowTechDocPage =
    (Number(AllTechDocPage[AllTechDocPage.length - 1][AllTechDocPage[AllTechDocPage.length - 1].length - 1].labelDisplayName) - 1) * showTechDocPage;
  let searchInput = {
    attributesToInflate: ['object_name', 'object_desc'],
    maxToLoad: showTechDocPage,
    maxToReturn: -1,
...
export async function pagingAfterFaqAction(data, ctx) {
  ...
  if (
    num ===
    data.dataProviders.faqChipDataProvider.viewModelCollection.loadedVMObjects[
      data.dataProviders.faqChipDataProvider.viewModelCollection.loadedVMObjects.length - 1
    ].labelDisplayName
  ) {
    data.dataProviders.faqChipDataProvider.viewModelCollection.loadedVMObjects.splice(0);
    for (let pageList of AllFaqPage) {
...
export async function pagingAfterAskExpertAction(data, ctx) {
  ...
  if (
    num ===
    data.dataProviders.askExpertChipDataProvider.viewModelCollection.loadedVMObjects[
      data.dataProviders.askExpertChipDataProvider.viewModelCollection.loadedVMObjects.length - 1
    ].labelDisplayName
  ) {
    data.dataProviders.askExpertChipDataProvider.viewModelCollection.loadedVMObjects.splice(0);
    for (let pageList of AllAskExpertPage) {
...
export async function lastPageAskExpertAction(data, ctx) {
  ...
  nowAskExpertPage =
    (Number(AllAskExpertPage[AllAskExpertPage.length - 1][AllAskExpertPage[AllAskExpertPage.length - 1].length - 1].labelDisplayName) - 1) * showAskExpertPage;
  let searchInput = {
    attributesToInflate: ['object_name', 'object_desc'],
    maxToLoad: showAskExpertPage,
    maxToReturn: -1,
