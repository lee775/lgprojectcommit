...
export async function pagingBeforeFrequencyAction(data, ctx) {
    ...
    let listArray = await query.performSearchViewModel4( null, searchInput, params );FrequencyListArray = listArray.searchResults;
    FrequencyListArray.sort((a, b) => new Number(b.props.l2_views.dbValues[0]) - new Number(a.props.l2_views.dbValues[0]));
...
export async function pagingAfterFrequencyAction(data, ctx) {
    ...
    if (num === data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects[data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects.length - 1].labelDisplayName) {
        data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects.splice(0);
        for (let pageList of AllFrequencyPage) {
            if ((Number(num) + 1) === Number(pageList[0].labelDisplayName)) {
                for (let page of pageList) {
...
export async function lastPageFrequencyAction(data, ctx) {
    ...
    nowFrequencyPage = (Number(AllFrequencyPage[AllFrequencyPage.length - 1][AllFrequencyPage[AllFrequencyPage.length - 1].length - 1].labelDisplayName) - 1) * showFrequencyPage;
    let searchInput = {
        "attributesToInflate": [ "object_name", "object_desc" ],
        "maxToLoad": -1,
        "maxToReturn": -1,
    ...
    data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects[data.dataProviders.frequencyChipDataProvider.viewModelCollection.loadedVMObjects.length - 1].selected = true;
