...
export async function loadUserList(data, ctx) {
    ...
    //     userData = [userData];
    // }
    await com.getProperties(userData, ["l2_is_expert", "l2_answer_count", "l2_point", "l2_knowledge_count", "l2_user_name", "l2_user_id", "l2_good_count", "owning_user"]);
    ...
    expert = await query.performSearchViewModel4(null, searchInput, params);
    expert = expert.searchResults;
...
export async function setPostList(response, data) {
    ...
    if (data.dataProviders.postList.viewModelCollection.loadedVMObjects.length === 0) {
        ...
        let userProp = []
        for(let owning of response) {
            userProp.push(cdmSvc.getObject(owning.props.owning_user.dbValues[0]));
        }
        await com.getProperties(userProp, ["awp0ThumbnailImageTicket"]);

        let vmoArray = [];
        for (let i = 0; i < 4; i++) {
            let vmo = vmos.constructViewModelObjectFromModelObject(response[i]);
            vmo.cellHeader1 = response[i].props.object_name.uiValues[0];
            ...
            vmo.rank = i + 1;
            // let userProp = cdmSvc.getObject(response[i].props.owning_user.dbValues[0]);
            // await com.getProperties(userProp, ["awp0ThumbnailImageTicket"]);
            let thumbnailUrl = userProp[i].props.awp0ThumbnailImageTicket.dbValues[0];
            if (!thumbnailUrl) {
...
export let createPieChart = function (data) {
    ...
    var dummyValuesForFirstSeries = [qna.length, faq.length, tech.length, expert.length];
    var seriesName = ["Q&A", "FAQ", technicalDocument, askExpert];
    var keyValueDataForChart = [];
...
export let createPointChartFromArrayOfSeries = async function () {
    ...
    for (let i = 0; i < result.length; i++) {
        dummyValuesForFirstSeries.push(Number(tech[i].props.l2_like_count.uiValues[0]));
    }
...
export async function updateDailyVisitorValue() {
