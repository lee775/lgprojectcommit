...
const loadColumnSeverity = (dataProvider) => {
  const columns = [
    constants.COL_GRADE,
    constants.COL_CAUSE_EFFECT,
    constants.COL_EFFECT,
    constants.COL_EVALUATION_STANDARD,
  ];
...
const loadColumnOccurence = (dataProvider) => {
  const columns = [
    constants.COL_GRADE,
    constants.COL_POSSIBILITY_OF_FAILURE,
    constants.COL_DESIGN_LIFE_RELIABILITY,
  ];
  _setColumns(dataProvider, columns);
};
...
const loadColumnDetection = (dataProvider) => {
  const columns = [
    constants.COL_GRADE,
    constants.COL_DETECTION_OPPORTUNITY,
    constants.COL_DETECTION_RANGE,
    constants.COL_DETECTABILITY,
  ];
  _setColumns(dataProvider, columns);
};
