...
export async function loadChecklistCreateTree(ctx, data) {
  if (!topItemRevision) {
    return [];
  }
...
export function loadChecklistCreateTreeData(modelObjects, nodeBeingExpanded) {
  const treeNodes = [];
...
export async function okAction(ctx, data) {
  ...
  lgepMessagingUtils.show(lgepMessagingUtils.INFORMATION, '체크리스트를 생성중입니다.\n잠시 기다려주세요.');
  // 체크리스트
  try {
