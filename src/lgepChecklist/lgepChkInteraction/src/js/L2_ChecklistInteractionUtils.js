...
export const getInteractionType = (grade) => {
  return INTERACTION_TYPES.filter((type) => grade === type.propInternalValue)[0]
    .propDisplayValue;
};
...
export const getInteractionTable = async (topObject) => {
  ...
  const interactionTable = await Promise.all(
    interactionTableUids.map(async (uid) => {
      return await loadObjectByPolicy(uid, TYPE_INTERACTION_ROW, [
        PROP_PRIMARY,
        PROP_SECONDARY,
        PROP_GRADE,
      ]);
...
export const getInteractionStructureInfo = () => {
  const structureList = appCtxService.ctx.checklist.structure;
  const topLine = structureList[0];
  const structureInfo = {
    [PARENT_ASSY]: [],
    [SUB_ASSY]: [],
  };
