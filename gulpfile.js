...
const { cloneDeep, forEach, merge, mergeWith, isArray, union } = require('lodash');
...
const { error, pathColor, pipeErrorHandler, Stopwatch, success, trace } = require('afx/build/js/logger');

const { normalizePath, shortenPath, spawn, stream2Promise, tapBlock } = require('afx/build/js/util');
...
async function createCache() {
  ...
  createCachePromise =
    createCachePromise ||
    stream2Promise(
      gulp.src(buildJson.srcPaths, { allowEmpty: true }).pipe(
        tapBlock(function (file) {
...
async function audit() {
  ...
  forEach(cache.filePath2moduleJson, function (moduleJson) {
    ...
    if (moduleJson.srcDir) {
      srcSCA = srcSCA.concat([`${moduleJson.srcDir}/*.@(css|scss)`, `${moduleJson.srcDir}/**/*.@(html|js|json|xml)`]);
  ...
  promises.push(
    stream2Promise(
      ...
      gulp.src(schemaPaths, { allowEmpty: true }).pipe(
        tap((file) => {
          if (file.contents) {
      ...
      if (srcSCA.length > 0) {
        return stream2Promise(gulp.src(srcSCA, { allowEmpty: true }).pipe(staticCodeAnalysis('', cache)));
      }
    }),
  );

  if (srcESLint.length > 0) {
    const gulpSrcESLint = gulp
      .src(srcESLint, { allowEmpty: true })
      .pipe(eslint())
 ...
 */
gulp.task('refresh', () => buildSite(true).catch(pipeErrorHandler));
...
gulp.task('genSoaApi', (cb) => {
  require('./build/js/genSoaApi').generate(() => {
    if (process.platform === 'win32') {
      spawn('cmd', ['/c', 'start', 'out/soa/api/index.html']).then(cb).catch(cb);
    } else {
      cb();
    }
  });
